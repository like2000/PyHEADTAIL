    ...
    *The time step is fixed to be one turn, but the tracking can consist of 
    ...
    def kick(self, beam):
        ...
        *The Kick represents the kick(s) by an RF station at a certain position 
        ...
        for i in range(self.n_rf):
            beam.dE[self.ni:self.nf] += self.voltage[i,self.counter[0]] * \
                                        np.sin(self.harmonic[i,self.counter[0]] * 
                                               beam.theta[self.ni:self.nf] + 
    ...
    def kick_acceleration(self, beam):
        ...
        *KickAcceleration gives a single accelerating kick to the bunch. 
        ...
        beam.dE[self.ni:self.nf] += self.acceleration_kick[self.counter[0]]
    ...
    def drift(self, beam):
        ...
        *The drift updates the longitudinal coordinate of the particle after 
        ...
        if self.solver == 'full': 
            beam.theta[self.ni:self.nf] = self.beta_ratio[self.counter[0]] * beam.theta[self.ni:self.nf] \
                                        + 2 * np.pi * (1 / (1 - self.rf_params.eta_tracking(beam.delta[self.ni:self.nf]) * 
                                                            beam.delta[self.ni:self.nf]) - 1) * self.length_ratio
        elif self.solver == 'simple':
            beam.theta[self.ni:self.nf] = self.beta_ratio[self.counter[0]] *beam.theta[self.ni:self.nf] \
                                        + 2 * np.pi * self.eta_0[self.counter[0]] \
                                        * beam.delta[self.ni:self.nf] * self.length_ratio
        else:
    ...
    def track(self, beam):
        ...
        # Parallel computing
        if self.mpi_conf != None:
            self.mpi_i = int(beam.n_macroparticles/self.mpi_size)
