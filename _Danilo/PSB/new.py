...
for i in range(n_turns):
    ...
    if (i % n_turns_between_two_plots) == 0:
        ...
        - 1e1, 1e1, xunit = 'ns', sampling = 10, separatrix_plot = True)
        plt.figure() 
        ax = plt.axes() 
...
np.savez_compressed('phase_space', my_beam.theta, my_beam.dE, n_turns)
np.savez_compressed('bunch_length', save_bunch_length)
np.savez_compressed('bunch_position', save_bunch_position)
np.savez_compressed('perc_particles_lost', save_perc_number_particles_lost)
...
plt.savefig('Percentage_particles_lost.png')
plt.clf()
print "Done!"
