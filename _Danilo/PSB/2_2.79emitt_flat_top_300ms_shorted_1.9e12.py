...
print 1 / general_params.f_rev[0] * 300000
sys.exit()
# DEFINE BEAM------------------------------------------------------------------
...
steps_plus_space_charge = InductiveImpedance(slice_beam, 
     ...
     -(376.730313462 * 2.58) / (2 * general_params.beta_r[0,0] * general_params.gamma_r[0,0]**2), general_params.f_rev[0])
...
matched_from_distribution_density(my_beam, full_tracker, {'type':'waterbag', 
            'parameters':[emittance, 1.5], 'density_variable':'density_from_H'}, 
            main_harmonic_option = 'lowest_freq', TotalInducedVoltage = 
            total_induced_voltage, n_iterations_input = 2)
...
for i in range(n_turns):
    ...
    if (i % n_turns_between_two_plots) == 0:
        ...
        - 1e1, 1e1, xunit = 'ns', sampling = 10, separatrix_plot = True, dirname = parameter)
