...
bunchmonitor = BunchMonitor('beam_shorted', n_turns+1, statistics = "Longitudinal")
...
for i in range(n_turns):
    ...
    if ((i+1) % n_turns_between_two_plots) == 0:
        ...
        - 1e1, 1e1, xunit = 'ns', sampling = 10, separatrix_plot = True, dirname = 'figSH')
         
        plot_induced_voltage_vs_bins_centers(i+1, general_params, total_induced_voltage, style = '-', dirname = 'figSH')
          
        plot_beam_profile(i+1, general_params, slice_beam, dirname = 'figSH')
         
    
plot_impedance_vs_frequency(n_turns, general_params, ind_volt_freq, 
           option1 = "single", style = '-', option3 = "freq_table", option2 = "spectrum", dirname = 'figSH')
plot_bunch_length_evol(my_beam, 'beam_shorted', general_params, n_turns, dirname = 'figSH')
plot_position_evol(n_turns, my_beam, 'beam_shorted', general_params, unit = None, dirname = 'figSH')
