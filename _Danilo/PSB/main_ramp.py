...
if os.path.exists('output'):    
    os.system('del /s/q '+ os.getcwd() +'\\output>null')
else:
    os.makedirs('output')

real_beta_program = np.loadtxt('ps_booster_impedances/real_beta_program.out')
time_turns = real_beta_program[:, 0]
momentum_program = np.loadtxt('ps_booster_impedances/momentum_program_0.05_1.4.txt')
time = momentum_program[:, 0] * 1e-3
momentum = momentum_program[:, 1] * 1e6
sync_momentum = np.interp(time_turns, time, momentum)
sys.exit()
...
for i in range(n_turns):
    ...
    if (i % n_turns_between_two_plots) == 0:
        ...
        - np.pi, np.pi, 
        - 0.1e1, 0.1e1, sampling = 10, separatrix_plot = False)
        plt.figure() 
        ax = plt.axes() 
        ...
        plt.savefig('output/%d_turn'%i+'wake.png')
        plt.clf()
    print time.clock() - t0
...
np.savez_compressed('output/phase_space', my_beam.theta, my_beam.dE, n_turns)
np.savez_compressed('output/bunch_length', save_bunch_length)
np.savez_compressed('output/bunch_position', save_bunch_position)
np.savez_compressed('output/perc_particles_lost', save_perc_number_particles_lost)
...
plt.savefig('output/length_evolution.png')
plt.clf()
...
plt.savefig('output/position_evolution.png')
plt.clf()
...
plt.savefig('output/Percentage_particles_lost.png')
plt.clf()
print "Done!"
